...
import { SHIRITORI_DICT, getComputerWord } from './shiritoriDict';

// 型定義
type GameMode = 'title' | 'select' | 'category' | 'quiz' | 'shiritori' | 'shiritoriMode' | 'result' | 'ranking';
type ShiritoriMode = 'player' | 'computer';
...
const App = () => {
  ...
  const [shiritoriMode, setShiritoriMode] = useState<ShiritoriMode>('player');
  ...
  const startShiritori = (mode: ShiritoriMode = 'player') => {
  ...
  const handleShiritoriSubmit = () => {
    ...
    // コンピューターモードの場合、コンピューターのターン
    if (shiritoriMode === 'computer') {
      setIsComputerThinking(true);
      setShiritoriInput(''); // 入力をクリア
      setTimeout(() => {
        const computerWord = getComputerWord(lastChar, newUsedWords);
        if (computerWord) {
          setComputerWord(computerWord);
          const newUsedWordsWithComputer = [...newUsedWords, computerWord];
          setUsedWords(newUsedWordsWithComputer);
          const computerLastChar = computerWord.charAt(computerWord.length - 1);
          setCurrentChar(computerLastChar);
          setShiritoriTimer(30);
          setShiritoriStartTime(Date.now());
          setIsComputerThinking(false);
          setComputerWord('');
          ...
          // コンピューターが負け
          setIsComputerThinking(false);
          setErrorMessage('コンピューターが答えられませんでした！あなたの勝ちです！');
          setTimeout(() => showShiritoriResult(), 2000);
        }
      }, 1500);
    } else {
  ...
  if (mode === 'shiritoriMode') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-500 to-teal-600 flex items-center justify-center p-4">
        <div className="max-w-4xl w-full">
          <h2 className="text-4xl font-bold text-white text-center mb-12 drop-shadow-lg">しりとりモード選択</h2>
          <div className="grid md:grid-cols-2 gap-8">
            {/* プレイヤーモード */}
            <div 
              onClick={() => startShiritori('player')}
              className="bg-white rounded-2xl p-8 cursor-pointer transform hover:scale-105 transition-all hover:shadow-2xl"
            >
              <div className="text-6xl mb-4 text-center">👤</div>
              <h3 className="text-3xl font-bold text-blue-600 mb-4 text-center">1人プレイ</h3>
              <ul className="space-y-2 text-gray-700 font-medium">
                <li className="flex items-center">
            ...
            {/* コンピューターモード */}
            <div 
              onClick={() => startShiritori('computer')}
              className="bg-white rounded-2xl p-8 cursor-pointer transform hover:scale-105 transition-all hover:shadow-2xl"
            >
              <div className="text-6xl mb-4 text-center">🤖</div>
              <h3 className="text-3xl font-bold text-red-600 mb-4 text-center">VS コンピューター</h3>
              <ul className="space-y-2 text-gray-700 font-medium">
                <li className="flex items-center">
                ...
                <li className="flex items-center">
                  <Timer className="mr-2 text-red-500" size={20} />
                  ターン制バトル
                </li>
                <li className="flex items-center">
              ...
              <button className="w-full mt-6 bg-red-500 text-white py-3 rounded-lg font-bold hover:bg-red-600">
                挑戦する
              </button>
            </div>
          </div>
          <button
            onClick={() => setMode('select')}
            className="block mx-auto mt-8 px-8 py-3 bg-white text-green-600 rounded-lg text-lg font-bold hover:bg-gray-100 transition-all shadow-lg"
          >
            ← 戻る
          </button>
        </div>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-400 to-green-600 flex items-center justify-center p-4">
        <div className="max-w-2xl w-full bg-white rounded-2xl p-8 shadow-2xl">
          ...
          {/* コンピューター思考中メッセージ */}
          {isComputerThinking && (
            <div className="bg-yellow-100 border-2 border-yellow-400 rounded-lg p-6 mb-6 text-center animate-pulse">
              <div className="text-3xl mb-2">🤖</div>
              <div className="text-xl font-bold text-gray-800">コンピューターが考えています...</div>
            </div>
          )}
          ...
          <div className="bg-green-50 rounded-lg p-8 mb-6 text-center border-2 border-green-200">
            <div className="text-gray-700 mb-2 font-semibold">
              {shiritoriMode === 'computer' ? (isComputerThinking ? 'コンピューターのターン' : 'あなたのターン') : '次は...'}
            </div>
          ...
          <div>
            ...
            <button
              ...
              className="w-full bg-green-500 text-white py-4 rounded-lg text-xl font-bold hover:bg-green-600 transition-all shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={isComputerThinking}
            >
          ...
          {usedWords.length > 0 && (
            <div className="mt-6">
              ...
              <div className="flex flex-wrap gap-2">
                {usedWords.slice(-10).reverse().map((word, index) => {
                  const originalIndex = usedWords.length - 1 - index;
                  const isPlayerWord = shiritoriMode === 'computer' ? originalIndex % 2 === 0 : true;
                  return (
                    <span 
                      key={index} 
